...
import { getClasses } from '../services/timetableService';
...
function Dashboard() {
    ...
    useEffect(() => {
        ...
        const loadData = async () => {
            if (!user) return;
            try {
                const [nextTasks, nextClasses] = await Promise.all([getTasks(), getClasses()]);
                if (active) {
                    setTasks(nextTasks);
    ...
    const attendancePct = classes.length ? Math.round((completedTasks / Math.max(classes.length, 1)) * 100) : 92;
        ...
        <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
            ...
            <div className="mt-6 grid gap-4 lg:grid-cols-3">
                <div className="rounded-[20px] border border-[#2d3850] bg-[#171f33] p-5">
                    ...
                    <p className="mt-2 text-3xl font-semibold text-[#f4f7ff]">{classes.length}</p>
                </div>
                <div className="rounded-[20px] border border-[#2d3850] bg-[#171f33] p-5">
            ...
            <div className="mt-6 grid gap-4 xl:grid-cols-[1.2fr_0.8fr]">
                <div className="rounded-[20px] border border-[#2d3850] bg-[#171f33] p-5">
                    ...
                    <div className="mt-4 space-y-2">
                        {classes.length ? classes.slice(0, 3).map((item) => (
                            <div key={item.$id || item.id} className="flex items-center justify-between rounded-2xl border border-[#2d3850] bg-[#0f1424] px-3 py-3">
                                <div>
                ...
                <div className="rounded-[20px] border border-[#2d3850] bg-[#171f33] p-5">
                    ...
                    <div className="mt-4 space-y-2">
                        {tasks.length ? tasks.slice(0, 4).map((task) => (
                            <div key={task.$id || task.id} className="rounded-2xl border border-[#2d3850] bg-[#0f1424] px-3 py-3">
